function [indexes, win_start, win_end, max_mean] = runAdwin_ICIP(data, fi, p)
%set parameters
...
%% code to convert 
GT = [0,347, 569, 838, 967, 1314, 1536,1805, 1934];
data_new=[];
for i=2:length(GT)
    indd = randperm(GT(i)-GT(i-1));
    temp_data = data(GT(i-1)+1:GT(i),:);
    temp_data = temp_data(indd, :);
    data_new=[data_new; temp_data];
end
data = data_new;
%%
